...
def is_pil_image_list(image_list: List[Any]) -> bool:
    ...
    return (
        ...
        and all(isinstance(img, Image.Image) for img in image_list)
    )
...
def is_pil_image_batch_list(image_batch_list: MultiImageBatch) -> bool:
    ...
    """
    return (
        isinstance(image_batch_list, list)
        and len(image_batch_list) > 0
        and all(is_pil_image_list(batch) for batch in image_batch_list)
    )
...
def is_multi_image_batch(image_batches: Any) -> bool:
    ...
    # List[ImageBatch]
    if (
        not isinstance(image_batches, list) or len(image_batches) == 0
...
def numpy_list_to_pil_image(numpy_list: List[np.ndarray]) -> List[Image.Image]:
    ...
    # Squeeze batch dim if present
    squeezed = [
        arr.squeeze(0) if arr.ndim == 4 and arr.shape[0] == 1 else arr for arr in numpy_list
    ]
...
def standardize_image_batch(
    ...
    if isinstance(images, (torch.Tensor, np.ndarray)) and images.ndim == 3:
        images = (
            images.unsqueeze(0) if isinstance(images, torch.Tensor) else images[np.newaxis, ...]
        )
